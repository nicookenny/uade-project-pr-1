...
def mostrar_turnos_filtrados(dni_paciente, filtro):
    ...
    todos_turnos = list(filter(lambda t: t["paciente_dni"] == dni_paciente,Storage.Turnos.listar()))
    ...
    if filtro == "todos":
        ...
        turnos = [t for t in todos_turnos if tuple(t["fecha"]) >= fecha_hoy and t["estado"] == "Confirmado"]
    elif filtro == "pasados":
