...
def mostrarLista():
    ...
    for paciente in Datos.pacientes: # Cada vez que quieras llamar a un diccionario o una tupla le Agregas Datos.Nombre de la tupla o diccionario A usar 
        for id_paciente, datos in paciente.items():
            print(f"{id_paciente:<8} {datos['Nombre']:<20} {CalculoEdad(datos['Fecha de Nacimiento']):<12} {datos['Obra Social']} ")
...
def agregarPaciente():
    ...
    while True:
        ...
        dnisPacientes = [idPaciente for dicc in Datos.pacientes for idPaciente, datos in dicc.items()]
        if dni in dnisPacientes:
            nombre = [datos['Nombre'] for dicc in Datos.pacientes for idPaciente, datos in dicc.items() if idPaciente == dni]
            ...
            Datos.pacientes.append(CargaDeNuevPaciente(nombre,dni,FechaDeNacimiento,obrasocial))
